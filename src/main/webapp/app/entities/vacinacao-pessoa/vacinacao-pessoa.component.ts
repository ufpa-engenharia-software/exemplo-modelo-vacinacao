...
export class VacinacaoPessoaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.vacinacaoPessoaService
      ...
      .subscribe(
        (res: HttpResponse<IVacinacaoPessoa[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInVacinacaoPessoas(): void {
    this.eventSubscriber = this.eventManager.subscribe('vacinacaoPessoaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IVacinacaoPessoa[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/vacinacao-pessoa'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
